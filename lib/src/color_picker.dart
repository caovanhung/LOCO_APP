...
class CircleColorPicker extends StatefulWidget {
  ...
  /// Event listener that gets triggered when the user stops touching (on pan end).
  ...
  CircleColorPicker({
    ...
    this.radius = 120,
    this.initialColor = const Color(0xffff0000),
    this.thumbColor = const Color.fromARGB(255, 255, 255, 255),
    this.thumbRadius = 8,
    required this.colorListener,
    required this.onChangeEnd,  // Thêm tham số này
  }) : super(key: key);
...
class _CircleColorPickerState extends State<CircleColorPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanStart: (details) => handleTouch(details.globalPosition, context),
      onPanUpdate: (details) => handleTouch(details.globalPosition, context),
      onPanEnd: (_) => widget.onChangeEnd(),  // Gọi onChangeEnd khi thả tay ra
      child: Stack(
        children: <Widget>[
          SizedBox(
